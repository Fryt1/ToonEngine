	...
	FViewUniformShaderParameters& ViewUniformShaderParameters) const
{
	...
	if (Scene)
	{
		...
		if (ShouldRenderSkyAtmosphere(Scene, Family->EngineShowFlags))
		{
			...
			Flags |= SkyAtmosphereSceneProxy.IsHoldout() ? ENVCOMP_FLAG_SKYATMOSPHERE_HOLDOUT : 0;
		...
		if (ShouldRenderVolumetricCloud(Scene, Family->EngineShowFlags))
		{
			...
			Flags |= VolumetricCloudSceneProxy.bHoldout ? ENVCOMP_FLAG_VOLUMETRICCLOUD_HOLDOUT : 0;
	...
	//Toon
	if (GEngine->ToonRampTexture)
	{
		const FTextureResource* TextureResource = GEngine->ToonRampTexture->GetResource();
		if (TextureResource)
		{
			ViewUniformShaderParameters.ToonRampTexture = TextureResource->TextureRHI;
		}
	}
	//Toon
			...
			if (!View.bIsReflectionCapture && !View.bIsSceneCapture )
			{
				...
				if ((GAreScreenMessagesEnabled && !GEngine->bSuppressMapWarnings) && (bIsFrozen || bLocked || bStereoView || bShowAmbientCubemapMipGenSettingsWarning || bAnyWarning))
				{
					...
					AddDrawCanvasPass(GraphBuilder, {}, View, Output,
						...
						bShowSkinCacheOOM, bSingleLayerWaterWarning, bLightFunctionAtlasOutOfSlotWarning, bShowNoSkyAtmosphereComponentWarning, bFxDebugDraw, FXInterface, bShowLocalExposureDisabledWarning,
						...
						bRealTimeSkyCaptureButNothingToCapture, bShowWaitingSkylight, bShowAmbientCubemapMipGenSettingsWarning, bLocalFogVolumeInSceneButProjectDisabled, bMegaLightsHasWarning,
...
void FSceneRenderer::RenderThreadEnd(FRHICommandListImmediate& RHICmdList, const TArray<FSceneRenderer*>& SceneRenderers)
{
	...
	else
	{
		...
		if (SceneRenderCleanUpMode == ESceneRenderCleanUpMode::DeferredAndAsync)
		{
			...
			check(IsInRenderingThread());
			FGraphEventArray CommandListTasks = MoveTemp(GRHICommandList.WaitOutstandingTasks);
...
void AddResolveSceneDepthPass(FRDGBuilder& GraphBuilder, const FViewInfo& View, FRDGTextureMSAA SceneDepth)
{
		...
		[&View, SourceTexture, NumSamples, DepthExtent, ResolveRect](FRDGAsyncTask, FRHICommandList& RHICmdList)
	{
		...
		BatchedParameters.SetShaderTexture(TextureIndex, SourceTextureRHI);
